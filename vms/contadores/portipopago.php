...
if (isset($_POST['submit'])) {
    ...
    $detalledeventas = pg_query($db, "select h.id_sucursal,s.sucursal_nombre,h.fecha_cobro, sum(h.pago_efectivo) as efectivo, sum(h.pago_tarjeta) as tarjeta,sum(h.pago_puntos) as puntos ,sum(h.pago_credito) as credito,
...
group by h.fecha_cobro, h.id_sucursal, s.sucursal_nombre
order by h.fecha_cobro, h.id_sucursal;");
...
<button type="button" onclick="seleccionarTodas()">Seleccionar Todas</button>
		...
		<input type="date" id="fechainicial" name="fechainicial" class="form-control"  required></label>
		<label for="fechafinal">Hasta
		
		<input type="date" id="fechafinal" name="fechafinal" class="form-control"  required></label>
		</div>
		<div class="form-group">
		<label for="inputData">
																Seleccione Salida
 ...
 <div class="row">
    ...
    function setFormAction() {
        ...
        if (selectedOption == 1) {
    document.forms['marcas'].action = 'tipo_pago_excel.php';
