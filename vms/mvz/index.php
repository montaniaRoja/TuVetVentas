...
if (isset($_POST['enviar'])) {
	...
	$rolEmpleado="Medico Veterinario";


    
    
    // Consulta preparada para buscar usuarios por nombre o iniciales
    $query = "SELECT id, passwd, sucursal_asignada FROM tbl_empleados WHERE (iniciales_empleado = $1 OR nombre_empleado = $1) AND activo_sn = $2 and posicion=$3";
    ...
    $asignedSucursal= $row['sucursal_asignada'];
	$idUser=$row['id'];
    ...
    if (password_verify($password, $stored_password)) {
        ...
        if ($num2 > 0) {
            ob_start();
            $extra = "dashboard.php";
            $_SESSION['usuario'] = "MVZ";
            $_SESSION['login'] = $_POST['username'];
			$_SESSION['userid']=$idUser;
            $_SESSION['id'] = $row['id'];
            $_SESSION['sucursal'] = $asignedSucursal;
...
<html lang="en">
	<head>
		<title>Veterinarios Login</title>
		<meta charset="utf-8" />
	...
	<body class="login">
		<div class="row">
			<div class="main-login col-xs-10 col-xs-offset-1 col-sm-8 col-sm-offset-2 col-md-4 col-md-offset-4">
				...
				<h2>Ingreso Medico Veterinario</h2>
				</div>
