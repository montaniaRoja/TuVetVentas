...
class obtenerReportes{
    ...
    public $obtenerTrasladosSalidasEntradas;
    ...
    public function obtenerComprasDesglosadas(){
        if($this->select == 2){
            ...
            echo '<table border="1">';
            echo '<tr>';
            echo '<th colspan="7">Reporte Visitas</th>';
            echo '</tr>';
            echo '<tr>
                    <th>Factura</th>
                    <th>CodBarra</th>
    ...
    public function obtenerEvolucionCategorias(){
        if($this->select == 2){
            ...
            while($row = pg_fetch_object($query)){
                ...
                echo '<td>$'.number_format($row->monto_hace_5_mes, 2, '.', ',').'</td>';
                echo '<td>$'.number_format($row->monto_hace_4_mes, 2, '.', ',').'</td>';
                echo '<td>$'.number_format($row->monto_hace_3_mes, 2, '.', ',').'</td>';
                echo '<td>$'.number_format($row->monto_hace_2_mes, 2, '.', ',').'</td>';
                echo '<td>$'.number_format($row->monto_hace_1_mes, 2, '.', ',').'</td>';
                echo '<td>$'.number_format($row->monto_mes_actual, 2, '.', ',').'</td>';
                echo '</tr>';
            }
    ...
    public function obtenerTraslados(){
        if($this->select == 2){
            ...
            h.codigo,
            h.fecha_enviado,
            (select s.sucursal_nombre from sucursales s where s.id = h.cod_origen) as suc_origen,
            (select s.sucursal_nombre from sucursales s where s.id = h.cod_destino) as suc_destino,
            d.cod_barra,
            ...
            WHERE (h.cod_origen IN ($this->sucursal) or 
            h.cod_destino IN ($this->sucursal)) 
            AND (h.fecha_enviado BETWEEN '$this->ini' AND '$this->fin' OR h.fecha_recibido BETWEEN '$this->ini' AND '$this->fin')
    ...
    public function obtenerTrasladosSalidasEntradas(){
        if($this->select == 2){
            $query=("SELECT
            h.codigo,
            h.fecha_enviado,
            ...
            (select pd.prod_nombre from productos pd where pd.prod_codbarra = d.cod_barra) as producto,
            d.cantidad,
            (select pd.prod_costopromedio from productos pd where pd.prod_codbarra = d.cod_barra) as costo,
            (select m.marca_nombre from productos pd inner join marcas m on m.id = pd.id_marca where pd.prod_codbarra = d.cod_barra) as marca,
            h.estado 
            FROM dtraslados AS d
            INNER JOIN htraslado h ON h.id = d.id_traslado 
            WHERE (h.cod_origen IN ($this->sucursal) or 
            ...
            echo '<table border="1">';
            echo '<tr>';
            echo '<th colspan="11">Reporte Traslados Salidas Entradas</th>';
            echo '</tr>';
            echo '<tr>
...
/*=============================================
BUSCAR TRASLADOS SALIDAS ENTRADAS
=============================================*/ 
if(isset($_POST["buscarTrasladosSalidasEntradas"])){
    $obtenerTrasladosSalidasEntradas = new obtenerReportes();
    $obtenerTrasladosSalidasEntradas -> db = $db;
    $obtenerTrasladosSalidasEntradas -> ini = $_POST["ini"];
    $obtenerTrasladosSalidasEntradas -> fin = $_POST["fin"];
    $obtenerTrasladosSalidasEntradas -> sucursal = $_POST["sucursal"];
    $obtenerTrasladosSalidasEntradas -> select = $_POST["select"];
    $obtenerTrasladosSalidasEntradas -> obtenerTrasladosSalidasEntradas();
}