...
if (isset($_POST["borrar"])) {
    ...
    $borrar="delete from entradas where id=$1";
...
//actualizar a procesado
    ...
    $query = "UPDATE entradas SET estado=$1, usuario_autoriza=$3 WHERE id=$2";
    $params = array($estado, $idRecibido, $usuario);
    $actualizarEntrada = pg_query_params($db, $query, $params);
    
    if ($actualizarEntrada) {
        //echo "Entrada actualizada exitosamente";
    } else {
        echo "<script>alert('Error al actualizar la entrada: ');</script>";
        //echo "Error al actualizar la entrada: " . pg_last_error($db);
    }
    ...
    $Kardex="insert into kardex (id,sucursal_id, codbarra, doc_no, cant_ingreso, fecha_transaccion, descripcion) values ($1,$2,$3,$4,$5,$6,$7);";
    $params = array($siguienteId,$sucursalRecibida, $codRecibido,$docno,$cantidad,$fecha,$entrada);
