...
if (isset($_POST['submit'])) {
    ...
    $detalledeventas = pg_query($db, "select h.id_sucursal, s.sucursal_nombre ,h.creada_por, sum(h.pago_efectivo)+sum(h.pago_tarjeta)+sum(h.pago_credito) as total,
								count(pagada_sn) as conteo 
                                from hfacturas h
                                join sucursales s 
                                on h.id_sucursal=s.id
                                where h.pagada_sn=1 and fecha_cobro between '$fechaInicial' and '$fechaFinal'
                                AND h.anulada=0
...
<div class="row">
<div class="col-sm-8">
<h1 class="mainTitle">Ventas por Empleado</h1>
</div>
<ol class="breadcrumb">
<li>
<span></span>
...
<h5 class="over-title margin-bottom-15"><span class="text-bold">Ventas por Empleado</span></h5>
        ...
        while ($row = pg_fetch_object($detalledeventas)) {
            ...
            <tr>
				...
				<td><button type="button" class="btn btn-primary"
														...
														data-user="<?php echo $row->creada_por; ?>"
...
<div class="modal fade bd-example-modal-lg" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static">
    ...
    function setFormAction() {
        ...
        if (selectedOption == 1) {
    document.forms['marcas'].action = 'por_empleado_excel.php';
    ...
    document.forms['marcas'].action = '<?php echo $_SERVER['PHP_SELF']; ?>';
        ...
        $('#exampleModal').on('show.bs.modal', function(event) {
            ...
            console.log(user, fInicial, fFinal);
            $.ajax({
                type: 'POST',
                url: 'detalle_facturas_modal.php',
                data: {
                    user: user,
                    fInicial: fInicial,
                    fFinal: fFinal
