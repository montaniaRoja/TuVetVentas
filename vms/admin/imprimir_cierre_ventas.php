...
if ($is_localhost) {
    ...
    ob_start();
    $cierre=pg_query($db,"SELECT cobrada_por,fecha_cobro, SUM(pago_efectivo) as efectivo, SUM(pago_tarjeta) as tarjeta, sum(pago_credito) as credito, SUM(pago_puntos) as puntos
                        FROM hfacturas WHERE fecha_cobro = '$fechaImprimir'  and pagada_sn=1 and anulada=0 and id_sucursal='$sucursal'
    ...
    $pdf->Cell(30, 5, "Fecha de Venta ".$fechaImprimir ,0, 1, 'L');
    ...
    while($row = pg_fetch_object($cierre)){
        ...
        $pdf->Cell(30, 5, "Venta en Puntos----- ".$puntos, 0, 1, 'L');
    ...
    $granTotal=$totalCredito+$totalEfectivo+$totalTarjeta+$totalPuntos;
    ...
    $pdf->Cell(40, 5, "Venta Total Puntos----- ", 0, 0, 'L');
    $pdf->Cell(40, 5, $totalPuntos, 0, 1, 'R');
    ...
    $listadefacturas=pg_query($db,"select * from hfacturas where fecha_cobro='$fechaImprimir' and pagada_sn=1 and anulada=0 and id_sucursal='$sucursal'");
    
    
    $pdf->Cell(30, 5, "efect. tarjeta puntos credito total" , 0, 1, 'L');
    
    while($row = pg_fetch_object($listadefacturas)){
        $numeroFactura = $row->numero_impreso;
        $totalPagado=($row->pago_efectivo+$row->pago_tarjeta+$row->pago_credito+$row->pago_puntos);
        $pagoE = "$" . number_format($row->pago_efectivo, 2);
        $pagoT = "$" . number_format($row->pago_tarjeta, 2);
        $pagoP = "$" . number_format($row->pago_puntos, 2);
        $pagoC = "$" . number_format($row->pago_credito, 2);
        $totalLinea = "$" . number_format($totalPagado, 2);
        $total=$total+$row->total_linea;
        
        $pdf->Cell(17, 5, $numeroFactura, 0, 1, 'L');
        $pdf->Cell(17, 5, $pagoE, 0, 0, 'R');
        $pdf->Cell(17, 5, $pagoT, 0, 0, 'R');
        $pdf->Cell(17, 5, $pagoP, 0, 0, 'R');
    ...
    $facturasAnuladas=pg_query($db,"select * from hfacturas where fecha_anulacion='$fechaImprimir' and pagada_sn=1 and anulada=1 and id_sucursal='$sucursal'");
    ...
    $pdf->Output('cierre_ventas_' .$fechaImprimir. '.pdf', 'D');
