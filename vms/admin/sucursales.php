...
if(isset($_POST['submit']))
{
    ...
    $sucursalNombre=$_REQUEST['sucursal'];
    ...
    $params = array($sucursalNombre,$direccion,$telefono,$celular,$fecha_actual,$usuarioCrea );
    $resultado = pg_query_params($db, $query, $params);
    /*
    if(!$resultado){
        $error_message = pg_last_error($db);
        echo "<script>alert('NO SE CREARON LA SUCURSAL. Error: " . $error_message . "');</script>";
    }
    */
...
// Verifica si la consulta MAX(id) fue exitosa
if ($maxID !== false) {
    //echo "<script>alert('INTENTO ACTUALIZAR LAS EXISTENCIAS');</script>";
    $estado = "no asignado";
    $cantidad = 0;
    ...
    if ($result) {
        // Itera sobre los resultados de la consulta utilizando pg_fetch_assoc
        while ($fila = pg_fetch_assoc($result)) {
            $maxIdExistencias++;
            $maxIdPr++;
            $codigo = $fila['prod_codbarra'];
            $existencias = "INSERT INTO existencias(id,id_sucursal, codbarra, existencia, anaquel) VALUES ($1, $2, $3, $4,$5)";
            $params = array($maxIdExistencias,$maxID, $codigo, $cantidad, $estado);
            $result3 = pg_query_params($db, $existencias, $params);
            
            $reorden = "INSERT INTO puntosdereorden(id, id_sucursal, codbarra, ptoreorden, fecha_creacion, creadopor) VALUES ($1,$2,$3,$4,$5,$6)";
            $params = array($maxIdPr,$maxID, $codigo,$cantidad, $fecha_actual, $usuarioCrea);
            $result4 = pg_query_params($db, $reorden, $params);
            
            
        }
    } else {
        echo "Error al realizar la consulta de productos.";
    }
} else {
    echo "Error al obtener el valor máximo de ID de sucursales.";
}

    
      /*
        $reorden = "INSERT INTO puntosdereorden(id_sucursal, codbarra, ptoreorden, fecha_creacion, creadopor) VALUES ($1,$2,$3,$4,$5)";
        $params = array($maxID, $codigo,$cantidad, $fecha_actual, $usuarioCrea);
        $result4 = pg_query_params($db, $reorden, $params);
        if (!$result4) {
            $error_message = pg_last_error($db); // Obtiene el mensaje de error de PostgreSQL
            echo "<script>alert('NO SE ACTUALIZARON LAS reoirdenes. Error: " . $error_message . "');</script>";
        }
       */ 
// Fin de agregar
