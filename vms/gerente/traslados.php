...
{//if del submit
	check_login();
    // guardar estado
    $query=("UPDATE htraslado SET fecha_recibido= 'now()', estado= 'Ingresado', usuario_recibe= '$_SESSION[login]' WHERE id=$_REQUEST[id]");
    ...
    $query1=("UPDATE dtraslados SET status= 'Ingresado' WHERE id_traslado=$_REQUEST[id]");
    ...
    $query2=("SELECT
            ...
            WHERE dt.id_traslado = $_REQUEST[id]
        ");
    ...
    while ($a = pg_fetch_assoc($query2)) {
		...
		$query4=("INSERT INTO kardex(id, sucursal_id, codbarra, cant_ingreso, cant_salio, fecha_transaccion, descripcion)
				VALUES ($queryuid,$_REQUEST[id_sucursal],'$codbarra',$cantidad,0,'now()', 'Ingreso por Traslado')");
    ...
    //pg_close();
    if($query){
        echo "<script>alert('Se ingreso traslado');</script>";
    }
						...
						<div class="container-fluid container-fullw bg-white">
							<div class="row">
								<div class="col-md-12">
									...
									<h5 class="over-title margin-bottom-15">Gestionar <span class="text-bold">Traslados</span></h5>
									...
									<table class="table table-hover" id="tabla_compras">
										...
										<tbody>
											...
											while($row = pg_fetch_object($sql)){
												if($row->id_destino == $_SESSION[sucursal] || $row->id_origen == $_SESSION[sucursal]){
												?>
												...
												<tr>
													...
													<td>
														<?php 
															...
															if ($row->estado == 'Aprobado' && $row->id_destino == $_SESSION[sucursal]){
																echo '<form id="formulario" role="form" method="post" enctype="multipart/form-data">
																<input type="hidden" name="id" value="'.$row->id.'">
																<input type="hidden" name="id_sucursal" value="'.$row->id_destino.'">
																<button class="btn btn-warning btnIngresarTraslado" style="margin-left:5px;margin-top:5px;margin-button:5px;" type="button" id="enviar" onClick="confirmar(this)">
																Ingresar</button>
																</form>';
															}
														?>
