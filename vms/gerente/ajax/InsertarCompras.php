...
class InsertarCompras{
    ...
    public function insertarRegistroCompra(){
        ...
        $query=("INSERT INTO comprasproductos(proveedor_id, compra_condicion, compra_fecha, compra_nodoc, compra_subtotal,
            compra_montoiva, compra_montoretencion, compra_total, id_usuario)
        VALUES ($this->proveedor,'$this->condicion',now(),'$this->factura',$arrayg[tsubtotal],
        $arrayg[tiva], $arrayg[tretencion], $arrayg[ttotal], $_SESSION[login])");
        ...
        if($query){
            ...
            for ($i=0; $i < count($arrayl); $i++) {
                ...
                if ($query3) {
                    ...
                    $query5=("INSERT INTO kardex(sucursal_id, codbarra, doc_no, cant_ingreso, cant_salio, descripcion)
                            VALUES ('$_SESSION[sucursal]','$codbarra','$this->factura',$cantidad_total,0,'Ingreso por Compra')");
