      ...
      if (dat && dat.hasOwnProperty('mensaje') && dat.mensaje === 'No se encontró resultado') {
        ...
        if($('#select').val() == 2){
          ...
          for (let i = 0; i < data.length; i++) {
            totalc +=Number(data[i]['cantidad']);
            totalm +=Number(data[i]['monto']);
            table.row.add([ data[i]['compra_nodoc'], data[i]['nombre_producto'], data[i]['nombre_proveedor'], data[i]['compra_fecha'], data[i]['cantidad'].toLocaleString(), data[i]['monto'].toLocaleString(undefined, opciones)]).draw(false);
          };
          $('#footer').html('');
          $('#footer').append('<tr><th colspan="4">Total:</th><th>'+totalc.toLocaleString()+'</th><th>'+totalm.toLocaleString(undefined, opciones)+'</th></tr>');
        }else{
          const fileName = `reportecompras_`+$('#ini').val()+`_`+$('#fin').val()+`.xls`
          if (window.navigator.msSaveOrOpenBlob) {
                window.navigator.msSaveBlob(res, fileName)
